...
DF = load_dataset('app/source/train.csv')
...
encoder = load_object('app/source/encoder.pkl')
scaler = load_object('app/source/scaler.pkl')
knn_imputer = load_object('app/source/knn_imputer.pkl')
pipe = load_object('app/source/knn_preproc8.pkl')
model = load_object('app/source/model.pkl')
...
def check_main_na(client: pd.DataFrame):
    ...
    if bl:
        ...
        templ_cat_encoded = pd.DataFrame(
            encoder.transform(templ_cat), columns=categoric_subset)
        ...
        for col in categoric_subset:
            ...
            if col in ['poutcome', 'subscribed']:
                none_kodes[col] = -1
        for col in categoric_subset:
            templ_cat_encoded[col] = templ_cat_encoded[col].replace(
                none_kodes[col], None)

        # сбор предобработанных данных
        temple_encode = pd.concat(
            [templ_num_scaled, templ_cat_encoded], axis=1, ignore_index=True)
        temple_encode = pd.DataFrame(temple_encode)
        temple_encode.columns = numeric_subset+categoric_subset

        # заполенение пропусков
        templ_imputed = pd.DataFrame(knn_imputer.transform(
            temple_encode), columns=temple_encode.columns)

        # раскодировка катeгориальных и числовых признаков
        client_num_imputed = pd.DataFrame(
            scaler.inverse_transform(templ_imputed[numeric_subset]))
        client_cat_imputed = pd.DataFrame(
            encoder.inverse_transform(templ_imputed[categoric_subset]))
        client_imputed = pd.concat(
            [client_num_imputed, client_cat_imputed], axis=1, ignore_index=True)
        ...
        client_data = pd.DataFrame(pipe.transform(
            client_imputed), columns=numeric_subset+categoric_subset)[main_columns]
        client_info = client_main_imputed
        mess = "Недостающие важные данные заполнены методом ближайших пяти соседей."
    else:
        # форматирование признаков для модели
        client_data = pd.DataFrame(pipe.transform(
            templ), columns=numeric_subset+categoric_subset)[main_columns]
